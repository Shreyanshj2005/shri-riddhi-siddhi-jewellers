    ...
    <div className="min-h-screen bg-muted/30">
      <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-border bg-card px-4">
        ...
        <div className="flex items-center gap-2">
          <a href="/" target="_blank" rel="noreferrer" className="hidden items-center gap-1 text-xs text-muted-foreground hover:text-foreground sm:flex">
      ...
      <div className="flex">
        <aside className="hidden w-60 shrink-0 border-r border-border bg-card lg:block" style={{ minHeight: "calc(100vh - 3.5rem)" }}>
          <div className="sticky top-14">{nav}</div>
        </aside>
        {open && (
          <div className="fixed inset-0 top-14 z-30 bg-background/95 lg:hidden" onClick={() => setOpen(false)}>
            <div className="bg-card" onClick={(e) => e.stopPropagation()}>{nav}</div>
          </div>
        )}
